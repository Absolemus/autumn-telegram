...
Процедура ПриЗагрузкеБиблиотеки(Путь, СтандартнаяОбработка, Отказ)
	Если Осень <> Неопределено Тогда
		...
		ПутьДоЗаготовки = ОбъединитьПути(Путь, "src", "Заготовки", "ЗаготовкаТелеграм.os");
		ПодключитьСценарий(ПутьДоЗаготовки, "ЗаготовкаТелеграм");
		Осень.ДобавитьЗаготовкуДляАвтоИнициализации("ЗаготовкаТелеграм");
